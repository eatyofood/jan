...
def back_end_parsing(p,TALKMODE):
    ...
    if "give me a chart" in p:
        webbrowser.open_new('https://www.tradingview.com/chart/R1YhS6f0/')
    ...
    if "what time is it" in p:
        ...
        hour= ' ' + hour
        hour= hour.replace(' 0','').replace(' ','')
        words = 'the current time is now {} {}'.format(hour,minute)
        say(words)
    if "hashtag" in p:
        
        if "hashtag bravo" in p:
            typewords('##')
        elif "hashtag charlie" in p:
            typewords('###')
        elif "hashtag delta" in p:
            typewords('####')
        else:
            typewords('# ')
    if "say hello" in p:
    ...
    if "to date time" in p:
        typewords("pd.to_datetime(")
